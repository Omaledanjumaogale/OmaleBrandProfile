...
<section id="career" class="bg-surface border-y border-border px-6 lg:px-12 py-20">
	...
	<div class="relative pl-8 border-l border-gold/20 ml-4 space-y-10">
		{#each timeline as item, i}
			<div class="relative reveal delay-{i * 100}">
				<div
					class="absolute -left-[41px] top-[6px] w-4 h-4 bg-gold border-4 border-surface rounded-full shadow-[0_0_0_4px_rgba(201,168,76,0.1)]"
				></div>
				<div class="font-['Space_Mono'] text-[9px] tracking-[2px] text-gold uppercase mb-1.5">
					{item.period}
				</div>
				<div class="font-['Bebas_Neue'] text-2xl tracking-[1px] text-text mb-1">
					{item.role}
				</div>
				<div class="text-[13px] text-muted2 font-light italic">
					{item.org}
				</div>
				<div class="text-[12px] text-muted leading-[1.7] mt-2 font-light max-w-2xl">
					{item.desc}
				</div>
			</div>
		{/each}
